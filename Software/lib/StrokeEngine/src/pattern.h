...
/*!
  ...
  or first part of the thrust out (Negastive sensation)
*/
/**************************************************************************/
class SensualDistance : public Pattern {
  public:
    ...
    motionParameter nextTarget(unsigned int index) override {
        ...
        //Temp DEBUG
        Serial.println("Rörelse: " + String(phase));
        Serial.println("Stroke: " + String(_nextMove.stroke));
        Serial.println("Speed: " + String(_nextMove.speed));
        Serial.println("Acc: " + String(_nextMove.acceleration));
        Serial.println("Sensation: " + String(_sensation));
  ...
  protected:
    ...
    void _calculatePatternPositions() override {
        ...
        unsigned int stepsPerMM = (_stepsPerMM > 0) ? _stepsPerMM : 1;
        unsigned int desiredAccelStepsperS2_fast = (unsigned int)(desiredAccelMMperS2_fast * stepsPerMM);
        unsigned int desiredAccelStepsperS2_slow= (unsigned int)(desiredAccelMMperS2_slow * stepsPerMM);
        unsigned int safeMaxAccel = (_maxAcceleration > 0) ? _maxAcceleration : desiredAccelStepsperS2_slow;


        _useSlowIn = _sensation > 5;
        _useSlowOut = _sensation < -5;
        ...
        // 1. Beräkna den normala maxhastigheten och accelerationen
        _speedFast = _speedStepsPerS;

        // Den långsamma delen ska vara 25% av inställd hastighet
        _speedSlow = int(min(float(_speedFast * 0.25), float(maxSpeedMMperS_slow * stepsPerMM)));

        // Calculate desired max acceleration (fixade tillagda parenteser här)
        ...
        // 2. Beräkna hur mycket av slaget som ska köras SNABBT
        _distFast = int(_stroke / 2 * (abs(_sensation) / 100.0));
...
/*
  ...
  Disabled OSSM does not support patternTable?
*/
/**************************************************************************//*
static Pattern *patternTable[] = {
  ...
  new Insist("Insist"),
  new SensualDistance("Sensual based on distance"),
  new SensualSpeed("Sensual based on speed")
